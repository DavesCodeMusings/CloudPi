...
if ! [ -f ./${HOSTNAME}.cnf ]; then
  echo "Can't find config file. Expecting: ${HOSTNAME}.cnf"
  exit 1
fi
...
if [ "$REPLY" == "y" ] || [ "$REPLY" == "Y" ]; then
  sudo openssl req -new -x509 -newkey rsa:2048 -sha256 -nodes -keyout /etc/ssl/private/${HOSTNAME}.key -days 365 -out /etc/ssl/certs/${HOSTNAME}.crt -config ${HOSTNAME}.cnf
fi
